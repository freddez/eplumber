    ...
    def _setup_routes(self):
        ...
        async def get_sensors():
            ...
            for sensor in self.eplumber.sensord.values():
                ...
                try:
                    ...
                    safe_mean = float(mean_value) if mean_value is not None and isinstance(mean_value, (int, float)) else mean_value
                    safe_last = float(last_value) if last_value is not None and isinstance(last_value, (int, float)) else last_value
                    safe_values = [float(v) if isinstance(v, (int, float)) else v for v in values_list]
                    ...
                    print(f"Error serializing sensor {getattr(sensor, 'name', 'unknown')}: {e}")
                    continue
        ...
        async def get_sensor(sensor_name: str):
            try:
                ...
                raise HTTPException(status_code=500, detail=f"Error reading sensor: {str(e)}")
        ...
        async def get_rules():
            try:
                ...
                for rule in self.eplumber.rules:
                    ...
                    rule_dict = {
                        "action_name": str(rule.action.name),
                        "tests": rule_tests,
        ...
        async def update_config(request: Request):
            try:
                ...
                if not config_data:
                    return JSONResponse(content={"error": "No config data provided"}, status_code=400)
                
                # Validate config by trying to create Config object
                try:
                    models.Config(**config_data)
                except Exception as validation_error:
                    return JSONResponse(content={"error": f"Invalid config: {validation_error}"}, status_code=400)
                ...
                if self.eplumber._config_path:
                    ...
                    return JSONResponse(content={"error": "Config file path not found"}, status_code=500)
